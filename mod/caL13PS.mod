...
NEURON {
	...
	RANGE pbar, ica, mshift, hshift, qfact, hqfact
	:POINTER mu
}

PARAMETER {
	pbar = 1.7e-6    (cm/s)	: hold at vh = -100 mv, step to -30 mv
...
BREAKPOINT {
    ...
    ica  = ghk(v,cai,cao) * pbar * m * m * h	  : Kasai 92, Brown 93
	:What the hell is the following line?
    :mshift	= (1-mu)*10
