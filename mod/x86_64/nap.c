...
#define _f_rates _f_rates__nap 
#define rates rates__nap 
#define state state__nap 
...
extern "C" {
 ...
 static void _hoc_table_tauhnap(void);
 static void _hoc_tauhnap(void);
...
extern Memb_func* memb_func;
 ...
 "table_tauhnap_nap", _hoc_table_tauhnap,
 "tauhnap_nap", _hoc_tauhnap,
 0, 0
};
#define table_tauhnap table_tauhnap_nap
#define tauhnap tauhnap_nap
 extern double table_tauhnap( );
 extern double tauhnap( _threadargsprotocomma_ double );
...
#define hslope hslope_nap
 double hslope = 10;
#define hvhalf hvhalf_nap
 double hvhalf = -48.8;
#define mslope mslope_nap
 double mslope = -4.6;
#define mvhalf mvhalf_nap
 double mvhalf = -52.6;
#define qfact qfact_nap
 double qfact = 3;
#define usetable usetable_nap
 ...
 "mslope_nap", &mslope_nap,
 "hvhalf_nap", &hvhalf_nap,
 "hslope_nap", &hslope_nap,
 "qfact_nap", &qfact_nap,
...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_dparam_semantics(_mechtype, 3, "cvodeieq");
 	...
 	ivoc_help("help ?1 nap /export/home1/users/bssn/serna/HBP/SpineModel/mod/x86_64/nap.mod\n");
...
static int _reset;
static char *modelname = "NaP - persistent sodium current for nucleus accumbens ";
...
static void* _ptable_tauhnap = (void*)0;
 
double tauhnap ( _threadargsprotocomma_ double _lv ) {
 double _arg[1];
 _arg[0] = _lv;
 return hoc_func_table(_ptable_tauhnap, 1, _arg);
 }
/*  }
  */
...
static void _hoc_tauhnap(void) {
  ...
  _nt = nrn_threads;
 _r =  tauhnap ( _p, _ppvar, _thread, _nt, *getarg(1) );
 hoc_retpushx(_r);
}
 double table_tauhnap ( ) {
	hoc_spec_table(&_ptable_tauhnap, 1);
	return 0.;
}
...
/*CVODE*/
 static int _ode_spec1 (double* _p, Datum* _ppvar, Datum* _thread, _NrnThread* _nt) {int _reset = 0; {
   ...
   Dm = ( minf - m ) / mtau ;
   Dh = ( hinf - h ) / ( tauhnap ( _threadargscomma_ v ) / qfact ) ;
   }
 return _reset;
}
 ...
 rates ( _threadargscomma_ v ) ;
 Dm = Dm  / (1. - dt*( ( ( ( - 1.0 ) ) ) / mtau )) ;
 Dh = Dh  / (1. - dt*( ( ( ( - 1.0 ) ) ) / ( tauhnap ( _threadargscomma_ v ) / qfact ) )) ;
  return 0;
}
 /*END CVODE*/
 static int state (double* _p, Datum* _ppvar, Datum* _thread, _NrnThread* _nt) { {
   rates ( _threadargscomma_ v ) ;
    ...
    h = h + (1. - exp(dt*(( ( ( - 1.0 ) ) ) / ( tauhnap ( _threadargscomma_ v ) / qfact ))))*(- ( ( ( hinf ) ) / ( tauhnap ( _threadargscomma_ v ) / qfact ) ) / ( ( ( ( - 1.0 ) ) ) / ( tauhnap ( _threadargscomma_ v ) / qfact ) ) - h) ;
   }
  return 0;
}
