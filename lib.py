...
def init_active(model, axon=False, soma=False, dend=True, dendNa=False,
    ...
    if spne:
        for s in model.spne:
            ...
            s.insert('cal_ion')
            s.insert('caq_alt'); s.pcaqbar_caq_alt = 6.0e-6      
            # N - type
            #s.insert('can'); s.pbar_can = 1.0e-5      
            s.insert('can_alt'); s.pbar_can_alt = 1.0e-5      
            
            # (HVA) L - type
            #s.insert('caL'); s.pbar_caL = 6.7e-6      
            s.insert('caL_alt'); s.pbar_caL_alt = 6.7e-6     
            
            # (Cav1.3) L - type
            #s.insert('caL13'); s.pcaLbar_caL13 = 1.7e-6
            s.insert('caL13_alt'); s.pbar_caL13_alt = 1.7e-6     
            s.insert('cad'); 
            #s.ca = 0.0023
            #s.insert('cad')
            #s.ena = model.Ena
