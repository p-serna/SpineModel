...
for i in range(len(diameters)*3):
    diamt = diameters[i%len(diameters)]
    dendsh.diam = diamt
    dendA1.diam = diamt
    dendA2.diam = diamt   
    diamneck = sps['meanDneck'].mean()/1e3
    ...
    #print(i,'0')
    with open(folderstore+"gatinginfovslv31Diam_temp"+condition+shflag+".pickle","wb") as f:
        pickle.dump(aps,f)
...
with open(folderstore+"gatinginfovslv31Diam"+condition+shflag+".pickle","wb") as f:
    pickle.dump(aps,f)
